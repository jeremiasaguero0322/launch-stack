...
describe("callSerper", () => {
  ...
  describe("non-2xx response throws error", () => {
    it("throws with status and body text on 500", async () => {
      mockFetch.mockResolvedValue(
        makeErrorResponse(500, "Internal Server Error", "Server down"),
      );
    ...
    it("throws on 401", async () => {
      mockFetch.mockResolvedValue(
        makeErrorResponse(401, "Unauthorized", "Invalid key"),
      );
  ...
  describe("positional score calculation is correct", () => {
    it("first item has highest score, last has lowest (score = 1 - position/total)", async () => {
      ...
      const total = 3;
      expect(results[0]!.score).toBeCloseTo(1 - 1 / total);
      expect(results[1]!.score).toBeCloseTo(1 - 2 / total);
      expect(results[2]!.score).toBeCloseTo(1 - 3 / total);
      expect(results[0]!.score).toBeGreaterThan(results[1]!.score);
      expect(results[1]!.score).toBeGreaterThan(results[2]!.score);
    });

    it("uses rank within items that have links only", async () => {
      mockFetch.mockResolvedValue(
        makeOkResponse({
          news: [
            { link: "https://a.com" },
    ...
    it("uses index+1 when position is missing", async () => {
      mockFetch.mockResolvedValue(
        makeOkResponse({
          news: [{ link: "https://a.com" }, { link: "https://b.com" }],
        }),
      );
