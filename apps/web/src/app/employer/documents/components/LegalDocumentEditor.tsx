...
const TEMPLATE_ICON: Record<string, "shield" | "gavel" | "briefcase" | "doc" | "usercheck"> = {
  nda: "shield",
  employee_nda: "shield",
  privacy_policy: "shield",
  service_agreement: "briefcase",
  contractor_agreement: "briefcase",
  advisory_agreement: "briefcase",
  founders_agreement: "briefcase",
  safe: "briefcase",
  terms_of_service: "gavel",
  non_compete: "gavel",
  ip_assignment: "gavel",
  invention_assignment: "gavel",
  employment_contract: "usercheck",
  at_will_employment: "usercheck",
  termination_letter: "doc",
  severance_agreement: "doc",
};

function tplIcon(kind: string) {
  switch (kind) {
    case "shield":
      return <Shield className="h-3.5 w-3.5" strokeWidth={1.5} />;
    case "gavel":
      return <Gavel className="h-3.5 w-3.5" strokeWidth={1.5} />;
    case "briefcase":
      return <Briefcase className="h-3.5 w-3.5" strokeWidth={1.5} />;
    case "usercheck":
      return <UserCheck className="h-3.5 w-3.5" strokeWidth={1.5} />;
    default:
...
export function LegalDocumentEditor({
  ...
  const [tplCat, setTplCat] = useState<"all" | TplCategory>("all");
  ...
  // ── Template rail data (drives left rail) ──
  const templates = useMemo(() => {
    return Object.values(TEMPLATE_REGISTRY).map((t) => ({
      id: t.id,
      name: t.name,
      cat: TEMPLATE_CATEGORY[t.id] ?? ("commercial" as TplCategory),
      icon: TEMPLATE_ICON[t.id] ?? "doc",
    }));
  }, []);
  const categories = useMemo(() => {
    ...
    return order
      ...
      .map((id) => ({
        id,
        label: id === "all" ? "All" : CATEGORY_LABEL[id as TplCategory],
        count: counts[id] ?? 0,
      }));
  }, [templates]);

  const filteredTemplates = useMemo(() => {
    const term = tplSearch.trim().toLowerCase();
    return templates.filter((t) => {
      if (tplCat !== "all" && t.cat !== tplCat) return false;
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className={lt.rdLegal}>
        {/* Left: templates rail */}
        <aside className={lt.rdTplRail}>
          <div className={lt.rdTplRailHead}>
            <h2 className={lt.rdTplRailH}>Templates</h2>
            <p className={lt.rdTplRailSub}>
              {templates.length} active
              {activeTemplateName ? ` · editing ${activeTemplateName}` : ""}
            </p>
          </div>
          <div className={lt.rdSearch}>
            <Search />
            <input
          ...
          <div className={lt.rdTplCats}>
            {categories.map((c) => (
              <button
                key={c.id}
                type="button"
                className={cn(lt.rdChip, tplCat === c.id && lt.rdChipActive)}
                onClick={() => setTplCat(c.id)}
              >
                {c.label}{" "}
                <span style={{ color: "var(--ink-4)", marginLeft: 4 }}>{c.count}</span>
              </button>
            ))}
          </div>
          <div className={cn(lt.rdTplList, lt.scrollbar)}>
            {filteredTemplates.length === 0 ? (
              <div className={lt.rdTplEmpty}>No matching templates</div>
            ) : (
              filteredTemplates.map((t) => {
                ...
                return (
                  <button
                    key={t.id}
                    type="button"
                    className={cn(lt.rdTplItem, active && lt.rdTplItemActive)}
                    onClick={() => {
                      if (active) return;
                      onBack();
                    }}
                    title={
                      active
                        ? "Currently editing"
                        : "Switch template (returns to template picker)"
                    }
                  >
                    <span className={lt.rdTplItemIcon}>{tplIcon(t.icon)}</span>
                    <span className={lt.rdTplItemBody}>
                      <span className={lt.rdTplItemName}>{t.name}</span>
                      <span className={lt.rdTplItemCat}>{CATEGORY_LABEL[t.cat]}</span>
                    </span>
                    {active && <span className={lt.rdTplItemUsed}>EDITING</span>}
                  </button>
                );
              })
            )}
